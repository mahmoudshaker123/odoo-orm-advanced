...
class Property(models.Model):
    ...
    description = fields.Text(tracking=True, groups="app_one.property_manager_group")
    ...
    def action_open_change_state_wizard(self):
        ...
        action['context'] = dict(self.env.context, default_property_id=self.id)
        return action
    ...
    def get_properties(self):
        payload = dict()
        try:
            response = requests.get('http://localhost:8080/v1/properties', data=payload)
...
class PropertyLine(models.Model):
    ...
    #     action = self.env['ir.actions.actions']._for_xml_id('app_one.property_action')
    #     view_id = self.env.ref('app_one.property_view_form').id
